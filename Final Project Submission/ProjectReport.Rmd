...
The Breusch-Pagan test reveals that the data is not homoscedastic, with a p-value below 0.05 (0.0196), and the Shapiro-wilk test reveals that the data is not normal, with a p value < 0.05 (6.72e-09). A possible explination for these issues is that delays may have a volume dependent variability, where busier airports can have both extremely on time flights and extremely delayed flights, increasing variance, or the large volume of small airports, where delays may be sporadic and unpredictable. Furthermore, it's likely that most flights are on time, or have a very short delay, resulting in an extreme right skew, violating non normality. In order to minimise the effects that these violations have on our data analysis, using robust standard errors to account for heteroscedasticity, and a generalised additive model (GAM) to account for non-normality. We observe that busyness has no statistically significant correlation with flight delays (p-value of 0.5548, > 0.05). The GAM produced an F statistic of 0.192, which indicates that the smooth term is not significant, which is backed up by the R^2 value of near zero (-0.00701), meaning almost none of the variance can be explained by busyness.
