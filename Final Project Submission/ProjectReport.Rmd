...
dep_delay_resids <- sample(residuals(dep_delay_model), size = 1000)
...
```{r}
min_delay <- min(flights_clean$dep_delay, na.rm = TRUE)
flights_clean_dep_delay <- log(flights_clean$dep_delay - min_delay + 1)
dep_delay_model_log <- lm(log_dep_delay ~ factor(year), data = flights_clean_log)

set.seed(222)  # for reproducibility
dep_delay_log_resids <- sample(residuals(dep_delay_model_log), size = 1000)
...
arr_delay_resids <- sample(residuals(arr_delay_model), size = 1000)
...
```{r}
min_arr_delay <- min(flights_clean$dep_delay, na.rm = TRUE)
flights_clean_dep_delay <- log(flights_clean$dep_delay - min_arr_delay + 1)
arr_delay_model_log <- lm(log_arr_delay ~ factor(year), data = flights_clean_log)

qset.seed(444)  # for reproducibility
arr_delay_log_resids <- sample(residuals(arr_delay_model_log), size = 1000)
...
both_delay_resids <- sample(residuals(both_delay_model), size = 1000)
...
dep_delay_airline_resids <- sample(residuals(dep_delay_model_airline_filtered), size = 1000)
...
arr_delay_airline_resids <- sample(residuals(arr_delay_model_airline_filtered), size = 1000)
