...
(combined_clean %>%
  group_by(dataset) %>%
  summarise(start = min(time_hour.y), end = max(time_hour.y), flights = n()))
...
```
- This plot represents departure delays for the years 2013 and 2023 when there is rain and no rain. We can see that flights departing during raining conditions show wider and right skewed delay distributions compared to when there is no rain in 2013 and 2023. This shows that rain shows a increase in the severity of delays.
...
```
- Rainy months seem to be consistent across 2013 and 2023. More specifically, rain causes significantly higher delay averages from August to November in 2013. From 2013 to 2023, we see that seasonal vulnerability to rain.
...
```
- This plot shows the scatterplot distribution of precipitation and departure delays. In 2023, departure delays increased steadily with precipitation, indicated rainfall lead to longer delays. As precipitation increased the trend seems to die down once it reaches .4. This could be due to operational handling over storms. In 2013 however the departure delays seem to increase much faster and don't die down as precipitation increases. Overall, we see a lack of operational handling of storms as time has gone by.
...
```
- In this map plot, we can see a change in average delay times with respect to rain at different NYC airports. For example, in 2013, EWR has a higher average delay time with JFK having the least amount of average delay. In 2023, this switches to JFK having the highest average delay due to rain. This may be due differences in airport infrastructure/traffic/scheduling density between EWR and JFK as time passed from 2013 to 2023.
