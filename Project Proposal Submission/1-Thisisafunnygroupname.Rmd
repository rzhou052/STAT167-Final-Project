...
combined_weather <- bind_rows(
  nycflights13::flights %>% 
    left_join(nycflights13::weather, by = c("origin", "time_hour")) %>%
    mutate(year = 2013),
  
  nycflights23::flights %>% 
    left_join(nycflights23::weather, by = c("origin", "time_hour")) %>%
    mutate(year = 2023)
) %>%
filter(!is.na(dep_delay), !is.na(precip))  # Clean data

# Create the combined plot
ggplot(combined_weather, aes(x = precip, y = dep_delay, color = factor(year))) +
  geom_point(alpha = 0.05, size = 0.5) +  # Semi-transparent small points
  geom_smooth(method = "lm", se = TRUE) +  # Trend lines with confidence bands
  scale_color_manual(values = c("2013" = "blue", "2023" = "red")) +
  xlim(0, 0.5) +
  labs(title = "Departure Delays vs Precipitation (2013 vs 2023)",
